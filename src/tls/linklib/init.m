...
extern struct Library *ObjFWRTBase, *ObjFWBase, *ObjFWTLSBase;
...
ctor(void)
{
	...
	struct OFTLSLinklibContext ctx = {
	...
	if ((ObjFWTLSBase = OpenLibrary(OBJFWTLS_AMIGA_LIB,
	    OBJFWTLS_LIB_MINOR)) == NULL)
		error("Failed to open " OBJFWTLS_AMIGA_LIB " version %lu!",
		    OBJFWTLS_LIB_MINOR);

	if (!OFTLSInit(1, &ctx))
		error("Failed to initialize " OBJFWTLS_AMIGA_LIB "!", 0);
...
dtor(void)
{
	if (ObjFWTLSBase != NULL)
		CloseLibrary(ObjFWTLSBase);
}

CONSTRUCTOR_P(ObjFWTLS, 127)
{
	ctor();
